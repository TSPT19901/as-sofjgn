...
class Key:
    ...
    def load_keys(self, pub_file="public.pem", priv_file="private.pem"):
...
def display_and_execute():
    ...
    while True:
        ...
        if option == "1":
            ...
            with open("HistoryText.txt", "a", encoding="utf-8") as f:
                f.write(repr(encrypted) + "\n")

            print("\nEncrypted data:\n", encrypted)

        elif option == "2":
            if not os.path.exists("HistoryText.txt"):
                print("No history found.")
                continue

            with open("HistoryText.txt", "r", encoding="utf-8") as f:
                lines = f.readlines()
