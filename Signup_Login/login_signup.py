...
#using getpass to make blink when input password
...
#display option for users to create/login account
def sign_up_login():
    while True:
        ...
        if input_choice not in ["S", "L", "E"]:
            ...
            break
#load data from file for check. compare
dict_store_account = load_data("DATA/account.txt")
#function that use for create account
def sign_up(): #Sign up account
    #Get user input to create their account
    while True:
        username = input("Username: ")

        if len(username) > 20:#check ensure password not too long
            print("Your username too long!\n\n\n")
            continue
        if username in dict_store_account:#check if password is already exists
            print("Username already exists!!\n\n\n")
            continue
        break
    #Try to check strength password each user account ensure it strong 
    while True:
        ...
        if (has_number and has_special and has_upper and has_lower):
            ...
            with open(part, "w"):
                pass
            break
#using encapsulation to compare password
class authentication:
    def __init__(self, username, password):
        self.username = username
        self.__password = password
    
    def compare_private_password(self, password_attempt): #compare password to login account   
